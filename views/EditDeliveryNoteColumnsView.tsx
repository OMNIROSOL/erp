...
import { ChevronRight, Check, HelpCircle, FileText, Layout, Settings } from 'lucide-react';
import { cn } from '../utils/cn';

const EditDeliveryNoteColumnsView = () => {
    const navigate = useNavigate();

    const [columns, setColumns] = useState(() => {
        const saved = localStorage.getItem('delivery_note_columns');
        const defaultCols = [
            { id: 'Delivery date', label: 'Delivery date', visible: true },
            { id: 'Reference', label: 'Reference', visible: true },
            { id: 'Order number', label: 'Order number', visible: true },
            { id: 'Invoice number', label: 'Invoice number', visible: true },
            { id: 'Customer', label: 'Customer', visible: true },
            { id: 'Inventory Location', label: 'Inventory Location', visible: true },
            { id: 'Description', label: 'Description', visible: false },
            { id: 'Qty delivered', label: 'Qty delivered', visible: true },
    ...
    const handleUpdate = () => {
        localStorage.setItem('delivery_note_columns', JSON.stringify(columns));
        ...
        <div className="bg-slate-50 min-h-screen selection:bg-indigo-100 selection:text-indigo-900 font-sans p-8 md:p-12 animate-in fade-in duration-700">
            ...
            <div className="max-w-2xl mx-auto mb-10">
                <div className="flex items-center space-x-2 text-[10px] font-black text-indigo-500 uppercase tracking-[0.2em] mb-4">
                    <span className="cursor-pointer hover:text-indigo-600 transition-colors" onClick={() => navigate('/delivery-notes')}>Delivery Notes</span>
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-slate-500 font-medium text-sm">Select which data fields you want to see for Delivery Notes.</p>
                    </div>
                </div>
            </div>
            ...
            {/* Info Section */}
            <div className="max-w-xl mx-auto mt-12 p-6 bg-white rounded-[24px] border border-slate-100 flex items-start gap-4 shadow-sm">
                <div className="w-10 h-10 rounded-full bg-slate-50 flex items-center justify-center text-slate-400 shrink-0">
                    <Settings size={18} />
                </div>
                <div>
                    <h4 className="text-[14px] font-bold text-slate-900 mb-1">Local Configuration</h4>
                    <p className="text-[12px] text-slate-500 leading-relaxed font-medium">
                        Your column preferences are saved locally to your browser. This means your dashboard will stay exactly how you want it, even after refreshing the page or logging back in.
                    </p>
                </div>
            </div>
        </div>
...
export default EditDeliveryNoteColumnsView;
