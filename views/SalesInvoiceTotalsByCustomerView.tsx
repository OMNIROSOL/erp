...
interface SalesInvoiceTotalsReport {
  id: string;
  fromDate: string;
  toDate: string;
  description: string;
...
const mockReports: SalesInvoiceTotalsReport[] = [
  {
    id: 'sit-report-1',
    fromDate: '01.03.2026',
    toDate: '31.03.2026',
    description: 'Q1 Sales Performance Summary',
    totalAmount: 145800.50,
    customerCount: 12
  }
];

const SalesInvoiceTotalsByCustomerView: React.FC = () => {
  const navigate = useNavigate();
  const [searchQuery, setSearchQuery] = useState('');
  const [reports, setReports] = useState<SalesInvoiceTotalsReport[]>([]);
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setReports(mockReports);
      localStorage.setItem('sales_invoice_totals_reports', JSON.stringify(mockReports));
    }
  }, []);
  ...
  const columns = [
    {
      ...
      header: 'Actions',
      className: 'w-24',
      accessor: (row: SalesInvoiceTotalsReport) => (
        <div className="flex items-center gap-2">
            <button
                onClick={() => navigate(`/reports/sales-invoice-totals-by-customer/view/${row.id}`)}
            ...
            <button
                onClick={() => navigate(`/reports/sales-invoice-totals-by-customer/edit/${row.id}`)}
    ...
    },
    {
      ...
      accessor: (row: SalesInvoiceTotalsReport) => (
        <span className="text-[13px] font-black text-slate-600">{row.customerCount} Customers</span>
      )
    },
    {
        id: 'Total',
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center space-x-2 text-[10px] font-black text-blue-600 uppercase tracking-widest mb-2">
            ...
            <span className="text-slate-400 uppercase">Sales Invoice Totals by Customer</span>
          </div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight font-sans text-left">Sales Invoice Totals by Customer</h1>
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            onClick={() => navigate('/reports/sales-invoice-totals-by-customer/new')}
          >
...
export default SalesInvoiceTotalsByCustomerView;
