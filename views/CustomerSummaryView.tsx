...
const mockReports: CustomerSummaryReport[] = [
  {
    id: 'cs-report-1',
    fromDate: '01.03.2026',
    toDate: '31.03.2026',
    division: 'Main Division',
    description: 'Monthly Summary Portfolio',
    count: 124
  }
];

const CustomerSummaryView: React.FC = () => {
  const navigate = useNavigate();
  const [searchQuery, setSearchQuery] = useState('');
  const [reports, setReports] = useState<CustomerSummaryReport[]>([]);
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setReports(mockReports);
      localStorage.setItem('customer_summary_reports', JSON.stringify(mockReports));
    }
  }, []);
  ...
  const columns = [
    {
      ...
      header: 'Actions',
      className: 'w-24',
      accessor: (row: CustomerSummaryReport) => (
        <div className="flex items-center gap-2">
            <button
                onClick={() => navigate(`/reports/customer-summary/view/${row.id}`)}
            ...
            <button
                onClick={() => navigate(`/reports/customer-summary/edit/${row.id}`)}
    ...
    },
    {
        ...
        accessor: (row: CustomerSummaryReport) => (
          <span className="text-[13px] font-black text-slate-900">{row.count || 0}</span>
        )
    },
    {
      id: 'Description',
  ...
  const filteredReports = reports.filter(r => 
    ...
    (r.toDate || (r as any).date || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center space-x-2 text-[10px] font-black text-blue-600 uppercase tracking-widest mb-2">
            ...
            <span className="text-slate-400">Customer Summary</span>
          </div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight">Customer Summary</h1>
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            onClick={() => navigate('/reports/customer-summary/new')}
          >
            <Plus size={16} /> Create Summary
          </button>
        </div>
      </div>
