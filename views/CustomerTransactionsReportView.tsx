...
const mockReports: TransactionReport[] = [
  {
    id: 'tx-2026-001',
    fromDate: '01.04.2026',
    toDate: '30.04.2026',
    ...
    description: 'General Customer Transaction Audit - Q2',
    transactionCount: 145
  }
];

const CustomerTransactionsReportView: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      setReports(mockReports);
      localStorage.setItem('customer_transaction_reports', JSON.stringify(mockReports));
    }
  }, []);
  ...
  const columns = [
    {
      ...
      header: 'Actions',
      className: 'w-24',
      accessor: (row: TransactionReport) => (
        <div className="flex items-center gap-2">
            <button
                onClick={() => navigate(`/reports/customer-transactions/view/${row.id}`)}
            ...
            <button
                onClick={() => navigate(`/reports/customer-transactions/edit/${row.id}`)}
      ...
      accessor: (row: TransactionReport) => (
        <div className="flex flex-col">
            <span className="text-[13px] font-medium text-slate-800 tracking-tight leading-none">
                {row.fromDate || '01.04.2026'} — {row.toDate || '30.04.2026'}
            </span>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter mt-1 italic opacity-70">
                Transaction Range
            </span>
        </div>
      )
    ...
    },
    {
        ...
        header: 'Transactions',
        accessor: (row: TransactionReport) => (
          <span className="text-[13px] font-black text-slate-900">{row.transactionCount || 0}</span>
        )
    },
    {
      id: 'Description',
    ...
    <div className="p-8 space-y-8 animate-in fade-in duration-700 font-sans">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 font-sans">
        <div>
          <div className="flex items-center space-x-2 text-[10px] font-black text-blue-600 uppercase tracking-widest mb-2">
            ...
            <span className="text-slate-400 uppercase">Customer Transactions</span>
          </div>
          <div className="flex items-center gap-3">
            <h1 className="text-2xl font-bold text-slate-900 tracking-tight font-sans">Customer Transactions</h1>
        ...
        <div className="flex items-center gap-3">
          <button 
            ...
            onClick={() => navigate('/reports/customer-transactions/new')}
          >
      ...
      <div className="p-4 bg-indigo-50/50 rounded-2xl border border-indigo-100/50 flex items-center gap-3">
        ...
        <p className="text-[11px] text-indigo-700 font-bold uppercase tracking-wider">
          Transaction reports provide a chronological audit of all customer ledger entries within the selected period.
        </p>
      </div>
    </div>
  );
...
export default CustomerTransactionsReportView;
