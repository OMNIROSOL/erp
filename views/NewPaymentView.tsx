...
const NewPaymentView = () => {
    ...
    const [paidTo, setPaidBy] = useState('Contact');
    const [paidToContact, setPaidByContact] = useState('Customer');
    const [paidToOptional, setPaidByOptional] = useState('');
    const [paidFrom, setReceivedIn] = useState('Account');
    const [paidFromAccount, setReceivedInAccount] = useState('');
            ...
            try {
                ...
                if (id) {
                    ...
                    if (payment) {
                        ...
                        setPaidByContact(payment.paidToContact || 'Customer');
                        setPaidByOptional(payment.paidToOptional || '');
                        setReceivedInAccount(payment.paidFromAccount || '');
    ...
    const handleSave = async () => {
        ...
        const paymentData = {
            reference: reference,
            date: date,
            paidToContact: paidToOptional || paidToContact,
            paidFromAccount: paidFromAccount || 'Suspense',
            description,
        ...
        <div className="p-10 max-w-[1400px] mx-auto space-y-8 selection:bg-indigo-100 selection:text-indigo-900 font-sans animate-in fade-in duration-700">
            ...
            <div className="bg-white rounded-[40px] shadow-2xl shadow-slate-200/50 border border-slate-100 overflow-hidden">
                    ...
                    <section className="space-y-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-4">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex bg-slate-50 border border-slate-200 rounded-2xl overflow-hidden focus-within:ring-4 focus-within:ring-indigo-500/10 focus-within:border-indigo-500 transition-all group">
                                    <div className="bg-slate-100 flex items-center px-4 border-r border-slate-200 text-[10px] font-black text-slate-400 uppercase tracking-widest">{paidFrom}</div>
                ...
                <div className="bg-slate-50/50 p-10 border-t border-slate-100 flex items-center justify-between">
                    ...
                    >
                        <Save size={18} />
                        <span>{id ? 'Update Payment' : 'Complete Payment'}</span>
                    </button>
                </div>
            </div>
        </div>
